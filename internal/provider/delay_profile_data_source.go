...
func (d *DelayProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError(ClientError, fmt.Sprintf("Unable to read %s, got error: %s", delayProfileDataSourceName, err))
	...
	if err != nil {
		resp.Diagnostics.AddError(DataSourceError, fmt.Sprintf("Unable to find %s, got error: %s", delayProfileDataSourceName, err))
...
func findDelayProfile(id int64, profiles []*radarr.DelayProfile) (*radarr.DelayProfile, error) {
