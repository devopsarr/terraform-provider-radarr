		...
		Steps: []resource.TestStep{
			...
			// Create and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("radarr_notification_pushbullet.test", "api_key", "key1"),
			...
			// Update and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("radarr_notification_pushbullet.test", "api_key", "key2"),
				),
			},
			// ImportState testing
			{
				ResourceName:      "radarr_notification_pushbullet.test",
				ImportState:       true,
...
func testAccNotificationPushbulletResourceConfig(name, key string) string {
	return fmt.Sprintf(`
	resource "radarr_notification_pushbullet" "test" {
		on_grab                            = false
		on_download                        = false
		on_upgrade                         = false
