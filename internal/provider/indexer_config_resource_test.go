		...
		Steps: []resource.TestStep{
			...
			// Create and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("radarr_indexer_config.test", "rss_sync_interval", "20"),
					resource.TestCheckResourceAttrSet("radarr_indexer_config.test", "id"),
				),
			},
			// Unauthorized Read
			{
			...
			// Update and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("radarr_indexer_config.test", "rss_sync_interval", "30"),
				),
			},
			// ImportState testing
			{
				ResourceName:      "radarr_indexer_config.test",
				ImportState:       true,
...
func testAccIndexerConfigResourceConfig(rss int) string {
	return fmt.Sprintf(`
	resource "radarr_indexer_config" "test" {
		maximum_size = 0
		minimum_age = 0
		retention = 0
		rss_sync_interval = %d
