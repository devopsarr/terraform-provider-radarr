		...
		Steps: []resource.TestStep{
			// Create and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("radarr_notification_join.test", "priority", "0"),
					resource.TestCheckResourceAttrSet("radarr_notification_join.test", "id"),
				),
			},
			// Update and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("radarr_notification_join.test", "priority", "2"),
				),
			},
			// ImportState testing
			{
...
func testAccNotificationJoinResourceConfig(name string, priority int) string {
	return fmt.Sprintf(`
	resource "radarr_notification_join" "test" {
		on_grab                            = false
		on_download                        = false
		on_upgrade                         = false
