		...
		Steps: []resource.TestStep{
			...
			// Create and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("radarr_notification_discord.test", "avatar", "dog-picture"),
					resource.TestCheckResourceAttrSet("radarr_notification_discord.test", "id"),
				),
			},
			// Unauthorized Read
			{
			...
			// Update and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("radarr_notification_discord.test", "avatar", "cat-picture"),
				),
			},
			// ImportState testing
			{
				ResourceName:      "radarr_notification_discord.test",
				ImportState:       true,
...
func testAccNotificationDiscordResourceConfig(name, avatar string) string {
	return fmt.Sprintf(`
	resource "radarr_notification_discord" "test" {
		on_grab                            = false
		on_download                        = false
		on_upgrade                         = false
		on_rename                          = false
