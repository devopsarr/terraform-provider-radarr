...
func (t resourceQualityProfileType) GetSchema(ctx context.Context) (tfsdk.Schema, diag.Diagnostics) {
	return tfsdk.Schema{
		...
		Attributes: map[string]tfsdk.Attribute{
			...
			"min_format_score": {
				MarkdownDescription: "Minimum custom format score to grab",
				Optional:            true,
				Computed:            true,
				Type:                types.Int64Type,
			},
			"cutoff_format_score": {
				MarkdownDescription: "Custom score cutoff",
				Optional:            true,
				Computed:            true,
				Type:                types.Int64Type,
			},
			"format_items": {
				MarkdownDescription: "Custom format items",
				Optional:            true,
				Computed:            true,
				Attributes: tfsdk.SetNestedAttributes(map[string]tfsdk.Attribute{
					"format": {
					...
					"score": {
						MarkdownDescription: "Custom format score",
						Optional:            true,
						Computed:            true,
						Type:                types.Int64Type,
					},
...
func (r resourceQualityProfile) Delete(ctx context.Context, req tfsdk.DeleteResourceRequest, resp *tfsdk.DeleteResourceResponse) {
	...
	err := r.provider.client.DeleteQualityProfileContext(ctx, int64(state.ID.Value))
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read qualityprofiles, got error: %s", err))
		return
	}
...
func writeQualityProfile(profile *radarr.QualityProfile) *QualityProfile {
	...
	formatItems := *writeFormatItems(profile.FormatItems)
	...
	return &QualityProfile{
		...
		MinFormatScore:    types.Int64{Value: profile.MinFormatScore},
		CutoffFormatScore: types.Int64{Value: profile.CutoffFormatScore},
		Name:              types.String{Value: profile.Name},
		Language:          types.String{Value: profile.Language.Name},
		FormatItems:       formatItems,
		QualityGroups:     qualityGroups,
	}
}

func writeFormatItems(formats []*radarr.FormatItem) *[]FormatItem {
	output := make([]FormatItem, len(formats))
	for m, f := range formats {
		output[m] = FormatItem{
			Format: types.Int64{Value: int64(f.Format)},
			Score:  types.Int64{Value: int64(f.Score)},
...
func readQualityProfile(profile *QualityProfile) *radarr.QualityProfile {
	...
	return &radarr.QualityProfile{
		...
		Qualities:         qualities,
		MinFormatScore:    profile.MinFormatScore.Value,
		CutoffFormatScore: profile.CutoffFormatScore.Value,
		FormatItems:       []*radarr.FormatItem{},
		Language: &starr.Value{
			ID:   helpers.GetLanguageID(profile.Language.Value),
...
func readFormatItems(formats *[]FormatItem) []*radarr.FormatItem {
	output := make([]*radarr.FormatItem, len(*formats))
	for m, f := range *formats {
		output[m] = &radarr.FormatItem{
			Format: int(f.Format.Value),
			Name:   f.Name.Value,
			Score:  int(f.Score.Value),
		}
	}
	return output
}
