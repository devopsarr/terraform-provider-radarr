		...
		Steps: []resource.TestStep{
			...
			// Create and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("radarr_notification_gotify.test", "priority", "0"),
					resource.TestCheckResourceAttrSet("radarr_notification_gotify.test", "id"),
				),
			},
			// Unauthorized Read
			{
			...
			// Update and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("radarr_notification_gotify.test", "priority", "5"),
				),
			},
			// ImportState testing
			{
...
func testAccNotificationGotifyResourceConfig(name string, priority int) string {
	return fmt.Sprintf(`
	resource "radarr_notification_gotify" "test" {
		on_grab                            = false
		on_download                        = false
		on_upgrade                         = false
