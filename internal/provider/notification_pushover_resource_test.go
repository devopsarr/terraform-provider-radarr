		...
		Steps: []resource.TestStep{
			// Create and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("radarr_notification_pushover.test", "priority", "0"),
					resource.TestCheckResourceAttrSet("radarr_notification_pushover.test", "id"),
				),
			},
			// Update and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("radarr_notification_pushover.test", "priority", "2"),
				),
			},
			// ImportState testing
			{
				ResourceName:      "radarr_notification_pushover.test",
				ImportState:       true,
...
func testAccNotificationPushoverResourceConfig(name string, priority int) string {
	return fmt.Sprintf(`
	resource "radarr_notification_pushover" "test" {
		on_grab                            = false
		on_download                        = false
		on_upgrade                         = false
