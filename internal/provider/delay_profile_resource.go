...
func (r *DelayProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError(ClientError, fmt.Sprintf("Unable to create %s, got error: %s", delayProfileResourceName, err))
	...
	if !profile.Order.IsUnknown() {
		...
		if err != nil {
			resp.Diagnostics.AddError(ClientError, fmt.Sprintf("Unable to update %s, got error: %s", delayProfileResourceName, err))
...
func (p *DelayProfile) write(ctx context.Context, profile *radarr.DelayProfile) {
...
func (p *DelayProfile) read(ctx context.Context) *radarr.DelayProfile {
	tags := make([]int, len(p.Tags.Elements()))
	tfsdk.ValueAs(ctx, p.Tags, &tags)

	return &radarr.DelayProfile{
		ID:                     p.ID.ValueInt64(),
